  ...
  return (
    <Container className=" py-10 grid grid-cols-1 md:grid-cols-2 gap-10 ">
      ...
      <div className=" flex flex-col gap-4">
        ...
        <AddToCartButton
          product={product}
          className="font-semibold uppercase rounded-md w-full"
        />
        <div className=" bg-[#f7f7f7] flex flex-col items-center justify-center gap-2 p-5">
          <Image
      ...
      <div className=" p-10 bg-[#f7f7f7] md:col-span-2 flex items-center gap-10 ">
        {product?.reviews?.map((item) => (
          <div
            ...
            className=" bg-white/80 p-5 border border-gray-200 rounded-md hover:border-gray-300 hover:bg-white duration-200 flex flex-col gap-1"
          >
            ...
            <div className=" flex items-center">
              {Array?.from({ length: 5 })?.map((_, index) => (
                <MdStar
                  key={index}
                  className={`${index < item?.rating ? "text-yellow-500" : "text-gray-400"}`}
                />
              ))}
            </div>
          </div>
